...
export function applyCommonAttributes(element, xmlNode, styleName = null) {
    ...
    const styles = styleName ? State.getStyles() : {};
    const styleData = (styleName && styles && styles[styleName]) ? styles[styleName] : {};
    const getAttr = (attr) => xmlNode.getAttribute(attr) ?? styleData[attr] ?? null;
...
export function applyStyles(element, styleName, xmlNode) {
    ...
    const getFinalValue = (xmlAttrNames, styleAttrNames, globalDefaultKey, fallback = null) => {
        ...
        for (const name of xmlNames) {
            ...
            try {
                ...
                xmlVal = xmlNode.getAttribute(name);
            } catch (e) {
                return fallback;
            }
            if (xmlVal !== null) return xmlVal;
        }
        for (const name of styleNames) { const styleVal = styleData[name]; if (styleVal !== undefined) return styleVal; }
        if (globalDefaultKey && globalDefaults[globalDefaultKey] !== undefined) { return globalDefaults[globalDefaultKey]; }
        return fallback;
    };
    ...
    if (roundedRectRatio && (element.classList.contains('gui-rounded-rect') || isContainer)) {
        const elementW = parseFloat(element.style.width || xmlNode.getAttribute('w') || '0');
        const elementH = parseFloat(element.style.height || xmlNode.getAttribute('h') || '0');
