...
function handleOverlayMouseOver(event) {
    const qualifiedSpan = event.target.closest('.hl-color-box-wrapper, .hl-color-rgb-warning-box, .hl-filepath');
    if (qualifiedSpan) {
...
function handleOverlayIconClick(event) {
    ...
    if (parentSpan.classList.contains('hl-color-box-wrapper') || parentSpan.classList.contains('hl-color-rgb-warning-box')) {
        ...
        if (lowerFilePath.endsWith('.xml')) {
         ...
         if (DEBUG_INTERACTIONS) console.warn("[IconClick] Icon clicked, but parent is not a recognized color or filepath span.", parentSpan);
    }
}
