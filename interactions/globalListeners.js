...
export function setupGlobalListeners() {
    ...
    if (!document.body.dataset.clipboardListenersAttached) {
        initializeClipboardAndLayerListeners();
        document.body.dataset.clipboardListenersAttached = 'true';
    }
}
...
function renderInspectorContent(container, el) {
        ...
        <div style="display: flex; flex-direction: row; height: ${previewH}px; overflow: hidden;">
            ...
            <div style="flex-grow: 1; padding: 12px; overflow-y: auto; color: #fff; width: ${panelW}px; box-sizing: border-box; display: flex; flex-direction: column;">
                ...
                <div style="font-size: 12px; font-weight: bold; margin-bottom: 8px; color: #cbd5e0; flex-shrink: 0; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="display:flex; gap:6px; opacity:0; pointer-events:none; position:absolute;">
                        <input type="number" id="qi-input-x" value="${x}"><input type="number" id="qi-input-y" value="${y}">
                        <input type="number" id="qi-input-w" value="${w}"><input type="number" id="qi-input-h" value="${h}">
                    </div>
                </div>
    ...
    // Search filter logic for attribute list
    const searchInput = container.querySelector('#qi-attr-search');
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            const term = e.target.value.toLowerCase();
            container.querySelectorAll('.qi-attr-row').forEach(row => {
                const attrName = row.dataset.attrName ? row.dataset.attrName.toLowerCase() : '';
                const attrValInput = row.querySelector('.qi-attr-val-input');
                const attrVal = attrValInput ? attrValInput.value.toLowerCase() : '';
                
                if (attrName.includes(term) || attrVal.includes(term)) {
