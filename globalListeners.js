...
import * as State from './state.js'; // For setIsMouseButtonDown
import { getTextLogo, getSidebar, getDisclaimerModal, getAcknowledgeDisclaimerButton } from './domUtils.js';
import { handlePanEnd } from './mainContentInteractions.js'; // <<< ADDED IMPORT
import { flashModalBorder } from './modalManager.js'; // <<< ADDED IMPORT
...
export function setupGlobalListeners() {
    ...
    // Disclaimer Mist Dissolve Effect
    ...
    if (ackBtn && discModal && !document.body.dataset.disclaimerListenerAttached) {
        ackBtn.addEventListener('click', () => {
            discModal.classList.add('dissolve-mist');
            discModal.addEventListener('animationend', () => {
                discModal.classList.remove('visible');
                discModal.classList.remove('dissolve-mist');
    ...
    // Use document.body to store listener flags
