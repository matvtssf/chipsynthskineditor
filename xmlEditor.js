...
function checkAndRerender(instanceId) {
    ...
    const currentSkinRootDirectory = State.getCurrentSkinRoot();
    const editedFilePath = instance.filePath;

    // ADDED: Debug logging
    console.log(`[checkAndRerender DEBUG] Instance File Path: "${editedFilePath}", Current Skin Root Dir: "${currentSkinRootDirectory}"`);

    let expectedMainSkinFile = "";
    if (currentSkinRootDirectory && typeof currentSkinRootDirectory === 'string' && currentSkinRootDirectory.length > 0) {
        const separator = currentSkinRootDirectory.endsWith('/') ? '' : '/';
        expectedMainSkinFile = State.normalizePath(currentSkinRootDirectory + separator + 'gui.xml');
    } else {
        // This case means currentSkinRootDirectory is not set or invalid.
        // The comparison will likely fail, which is correct if we don't know the main skin.
        console.warn(`[checkAndRerender DEBUG] Current Skin Root Directory is empty or invalid. Cannot reliably determine main skin XML.`);
        expectedMainSkinFile = editedFilePath; // Default to self, will likely not trigger general skin reload unless it's specifically set as root
    }

    console.log(`[checkAndRerender DEBUG] Comparing "${editedFilePath}" with expected main skin file "${expectedMainSkinFile}"`);

    if (editedFilePath === expectedMainSkinFile) {
        if (DEBUG_OPEN_EDITOR) console.log(`[checkAndRerender] Edited file ${editedFilePath} is the current main skin XML. Triggering reload.`);
        const elements = instanceDomElements.get(instanceId);
        if (elements && elements.textarea) {
            const updatedXmlContent = elements.textarea.value;
            loadSkin(editedFilePath, updatedXmlContent) // Use editedFilePath, which is the full path
                .then(() => {
                    showToast(`Skin ${editedFilePath} reloaded with changes.`, 'info');
                })
                .catch(err => {
                    logError(`[checkAndRerender] Error reloading skin ${editedFilePath}:`, err);
                    showToast(`Error reloading skin ${editedFilePath}. See console.`, 'error');
                });
        } else {
            logError(`[checkAndRerender] Could not get updated content from textarea for instance ${instanceId} to reload skin.`);
            showToast('Internal error: Could not get text to reload skin.', 'error');
        }
    } else {
        console.log(`[checkAndRerender DEBUG] Paths do NOT match the expected main skin file. No reload triggered for instance ${instanceId}.`);
        if (DEBUG_OPEN_EDITOR) console.log(`[checkAndRerender] Edited file ${editedFilePath} is NOT the current main skin XML (${expectedMainSkinFile}). No immediate reload.`);
    }
}
...
export function openXmlEditor(filePath) {
    ...
    if (elements.contentContainer && elements.header) makeModalDraggable(elements.contentContainer, elements.header);
    if (elements.contentContainer && elements.resizeHandle) makeModalResizable(elements.contentContainer, elements.resizeHandle);
...
export function highlightXmlSyntax(rawXmlText, instanceId) {
    ...
    const rawTextRanges = [];
    const attributeQuoteInfoMap = new Map();
    ...
    try {
        ...
        while ((tagMatch = tagRegex.exec(rawXmlText)) !== null) {
            ...
            rawTextRanges.push({ type: 'element', name: tagName, start: tagNameStart, end: tagNameEnd });
            ...
            if (!tagMatch[1] && tagMatch[3]) {
                ...
                while ((attrMatchInRaw = attrRegexInRaw.exec(attributesString)) !== null) {
                    ...
                    rawTextRanges.push({ type: 'attribute', name: attrName, start: attrNameStartInRaw, end: attrNameEndInRaw });
                    ...
                    if (attrName.toLowerCase() === 'param') {
                        ...
                        rawTextRanges.push({type: 'param', name: valueRaw, start: valueStartInRaw, end: valueEndInRaw });
                    }
                }
            }
        }
        ...
        while((rawAttrMatch = attrFullRegexRaw.exec(rawXmlText)) !== null) {
            ...
            attributeQuoteInfoMap.set(rawAttrMatch.index, { name: attrNameRaw, startQuoteIndex: quoteStartIndexInRaw, endQuoteIndex: quoteEndIndexInRaw });
        }
    ...
    try {
        ...
        tempHighlighted = tempHighlighted.replace(attributeValueRegex,
            (match, attrNameEscaped, separatorEscaped, openingQuoteEscaped, valueInsideDoubleQuotesEscaped, closingQuoteEscaped, attributeMatchOffsetInEscapedText) => {
                ...
                let bestGuessRawAttrStartIndex = -1;
                let minDiff = Infinity;
                attributeQuoteInfoMap.forEach((rawAttrInfo, rawAttrActualStartIndex) => {
                    if (rawAttrInfo.name === attrNameEscaped) {
                        const estimatedOriginalStartForEscapedAttr = attributeMatchOffsetInEscapedText - (textForHighlighting.length - rawXmlText.length);
                        const currentDifference = Math.abs(rawAttrActualStartIndex - estimatedOriginalStartForEscapedAttr);
                        if (currentDifference < minDiff) { minDiff = currentDifference; bestGuessRawAttrStartIndex = rawAttrActualStartIndex;}
                    }
                });
                ...
                segments.forEach(seg => {
                    ...
                    if (seg.type === 'color_placeholder') {
                        ...
                        const rawQuoteInfo = attributeQuoteInfoMap.get(bestGuessRawAttrStartIndex);
                        colorRegionsForEscapedText.push({
                            ...
                            hex: seg.hex,
                            type: seg.colorType,
                            attrQuoteRawStart: rawQuoteInfo?.startQuoteIndex,
                            attrQuoteRawEnd: rawQuoteInfo?.endQuoteIndex
                        });
    ...
    try {
        tempHighlighted = tempHighlighted.replace(/(&lt;\/?)([\w.:-]+)/g, (match, prefixAndSlash, tagNameEscaped, offsetInEscaped) => {
            ...
            const rawElemRange = rawTextRanges.find(r =>
                r.type === 'element' &&
