...
import { renderCS01TextButton, renderCS01OnOffButton, renderImageButton, renderCS01Button } from './buttonRenderer.js?v=2';
...
export function render(tagName, xmlNode, parentHtmlElement, currentParams, sourcePath, mergedAttributes) {
    switch (tagName) {
        ...
        case 'CS01Button':
            return renderCS01Button(xmlNode, mergedAttributes, currentParams, sourcePath);
        case 'CS01RadioButtons':
            return renderCS01RadioButtons(xmlNode, mergedAttributes, currentParams, sourcePath);
...
function renderCS01RadioButtons(xmlNode, mergedAttributes, currentParams, sourcePath) {
    const container = document.createElement('div');
    container.classList.add('gui-radio-buttons-container');
    container.style.position = 'absolute';

    const paramId = mergedAttributes['param'] || (currentParams ? currentParams.paramOffset : 0);
    const vDefault = mergedAttributes['vdefault'] || '0';

    const colorText = mergedAttributes['color_text'] || mergedAttributes['colorText'] || mergedAttributes['color'] || '#ffffff';
    const colorBack = mergedAttributes['color_back'] || mergedAttributes['colorBack'] || mergedAttributes['backColor'] || mergedAttributes['backcolor'] || '#555555';

    for (const child of xmlNode.children) {
        if (child.tagName === 'OptionItem') {
            const btn = document.createElement('button');
            btn.classList.add('gui-radio-option-button');
            btn.style.position = 'absolute';
            btn.style.boxSizing = 'border-box';
            btn.style.cursor = 'pointer';
            ...
            const imgOn = child.getAttribute('image_on');
            const imgOff = child.getAttribute('image_off');
            const labelOn = child.getAttribute('label_on') || child.getAttribute('label') || '';
            const labelOff = child.getAttribute('label_off') || child.getAttribute('label') || '';
            const fontAttr = child.getAttribute('font');
            ...
            const updateVisuals = () => {
                ...
                btn.textContent = isSelected ? labelOn : labelOff;
                ...
                if (isSelected) {
                    ...
                    btn.style.color = DomUtils.parseColor ? DomUtils.parseColor(colorBack) : colorBack;
                }
            };
            ...
            btn.addEventListener('click', (e) => {
                ...
                container.querySelectorAll('.gui-radio-option-button').forEach(b => {
                    if (typeof b.updateVisualState === 'function') b.updateVisualState();
                });
                if (typeof window?.updateControllerVisibilities === 'function') {
                    window.updateControllerVisibilities();
            ...
            btn.updateVisualState = updateVisuals;
            updateVisuals();
            container.appendChild(btn);
        }
    }
    ...
    return {
        ...
        requiresRecursiveRender: false,
        postProcessFunction: (element) => {
            element.querySelectorAll('.gui-radio-option-button').forEach(b => {
                if (typeof b.updateVisualState === 'function') b.updateVisualState();
            });
        }
    };
